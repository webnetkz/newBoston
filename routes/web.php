...
Route::get('/changepassword', function(){
	...
	$user->password = \Hash::make('kz123456789');
	$user->save();
});
...
Route::get('/testemail', function(){
    ...
    \Mail::send(['text'=>'mail'], $data, function($message) {
        $message->to('werewolflikan@gmail.com', 'Tutorials Point')->subject
        ('Laravel Basic Testing Mail');
        $message->from('werewolflikan@gmail.com','Virat Gandhi');
    });
    echo "Basic Email Sent. Check your inbox.";

});
Route::get('/account/checkpayment', 'PaypalController@checkpayment');
Route::get('/account/chk', function(){

    $headers = [
        'Authorization: Basic '. base64_encode("atamura:BpowZVnbDsOaHe8B"),
        'Content-Type: application/json',
        'Accept-Language: ru',
    ];

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL,"https://api.paymtech.kz/orders/".request()->order_id);
    curl_setopt($ch, CURLOPT_SSLCERT, "/var/www/vhosts/atamuraweb.kz/oqulyqtar.atamura.kz/storage/atamura.pem");
...
Route::get('/cert', 'ProtectedVideoController@getcert');
Route::get('/profile', function(){
    return view('my-cabinet');
});
Route::get('/payinfo', function(){
    return view('payinfo');
});
...
Route::middleware(['IsInstalled', 'isActive', 'switch_languages', 'ip_block', 'comming_soon'])->group(function () {
    ...
    //техподдержка остваить
    Route::post('comment/store', 'CommentController@store')->name('comment.store');

    //отзыв все
    Route::get('feedback/index', 'MovieCommentController@index');
    //отзыв оставить
    Route::post('feedback/store', 'MovieCommentController@store')->name('movie.comment.store');
   ...
   // Route::delete('movie/comment/delete/{id}', 'MovieCommentController@deletecomment')->name('movie.comment.delete');
    ...
    Route::get('password/reset/{token}', 'Auth\ForgotPasswordController@showResetPasswordForm')->name('password.reset');
